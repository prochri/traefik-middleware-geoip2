package traefikmiddlewaregeoip2_test
...
import (
	...
	mw "github.com/prochri/traefik-middleware-geoip2"
)
...
func TestGeoIPFromRemoteAddr(t *testing.T) {
	...
	headers := mw.CreateConfig().Headers
	assertHeader(t, req, headers.Country, "DE")
	assertHeader(t, req, headers.Region, "BY")
	assertHeader(t, req, headers.City, "Munich")

	req = httptest.NewRequest(http.MethodGet, "http://localhost", nil)
	req.RemoteAddr = "qwerty:9999"
	instance.ServeHTTP(httptest.NewRecorder(), req)
	assertHeader(t, req, headers.Country, mw.Unknown)
	assertHeader(t, req, headers.Region, mw.Unknown)
	assertHeader(t, req, headers.City, mw.Unknown)
}
